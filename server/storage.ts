import { 
  users, assets, transactions, prices, dividends, cashMovements, portfolioSnapshots, assetSnapshots,
  type User, type InsertUser, type Asset, type InsertAsset, 
  type Transaction, type InsertTransaction, type Price, type InsertPrice,
...
export interface IStorage {
  ...
  updateUserPortfolioName(userId: number, portfolioName: string): Promise<void>;

  // Asset operations
  getAssetsByUserId(userId: number): Promise<Asset[]>;
  getAsset(id: number, userId: number): Promise<Asset | undefined>;
  createAsset(asset: InsertAsset & { userId: number }): Promise<Asset>;
  updateAsset(id: number, userId: number, updates: Partial<InsertAsset>): Promise<Asset | undefined>;
  deleteAsset(id: number, userId: number): Promise<boolean>;

  // Transaction operations
  getTransactionsByUserId(userId: number): Promise<Array<Transaction & { asset: Asset }>>;
  ...
  // Price operations
  getPricesByUserId(userId: number): Promise<Array<Price & { asset: Asset }>>;
  getLatestPrices(userId: number): Promise<Array<Price & { asset: Asset }>>;
  createPrice(price: InsertPrice & { userId: number }): Promise<Price>;
  updatePrice(id: number, userId: number, updates: Partial<InsertPrice>): Promise<Price | undefined>;

  // Dividend operations
  getDividendsByUserId(userId: number): Promise<Array<Dividend & { asset: Asset }>>;
  createDividend(dividend: InsertDividend & { userId: number }): Promise<Dividend>;
  updateDividend(id: number, userId: number, updates: Partial<InsertDividend>): Promise<Dividend | undefined>;
  deleteDividend(id: number, userId: number): Promise<boolean>;

  // Cash movement operations
  getCashMovementsByUserId(userId: number): Promise<CashMovement[]>;
  createCashMovement(cashMovement: InsertCashMovement & { userId: number }): Promise<CashMovement>;
  updateCashMovement(id: number, userId: number, updates: Partial<InsertCashMovement>): Promise<CashMovement | undefined>;
  deleteCashMovement(id: number, userId: number): Promise<boolean>;

  // Portfolio snapshot operations
  getPortfolioSnapshots(userId: number): Promise<PortfolioSnapshot[]>;
  createPortfolioSnapshot(snapshot: InsertPortfolioSnapshot & { userId: number }): Promise<PortfolioSnapshot>;
  updatePortfolioSnapshot(id: number, userId: number, updates: Partial<InsertPortfolioSnapshot>): Promise<PortfolioSnapshot | undefined>;
  deletePortfolioSnapshot(id: number, userId: number): Promise<boolean>;

  // Asset snapshot operations
  getAssetSnapshots(userId: number, assetId: number): Promise<AssetSnapshot[]>;
  createAssetSnapshot(snapshot: InsertAssetSnapshot & { userId: number }): Promise<AssetSnapshot>;
  updateAssetSnapshot(id: number, userId: number, updates: Partial<InsertAssetSnapshot>): Promise<AssetSnapshot | undefined>;
  deleteAssetSnapshot(id: number, userId: number): Promise<boolean>;
  generateAssetSnapshots(userId: number, assetId: number): Promise<AssetSnapshot[]>;

  // Portfolio analytics
  getPortfolioOverview(userId: number): Promise<{
    totalValue: number;
  ...
  // Dividends summary
  getDividendsSummary(userId: number): Promise<{
    ytd: number;
    thisMonth: number;
    avgMonthly: number;
  }>;
  ...
  // Closed positions
  getClosedPositions(userId: number): Promise<Array<{
    asset: Asset;
    totalBought: number;
    totalSold: number;
    avgBuyPrice: number;
  ...
  deleteClosedPosition(userId: number, cycleId: string): Promise<boolean>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUserPortfolioName(userId: number, portfolioName: string): Promise<void> {
  ...
  async getAsset(id: number, userId: number): Promise<Asset | undefined> {
  ...
  async updateAsset(id: number, userId: number, updates: Partial<InsertAsset>): Promise<Asset | undefined> {
  ...
  async deleteAsset(id: number, userId: number): Promise<boolean> {
    const result = await db.delete(assets).where(and(eq(assets.id, id), eq(assets.userId, userId)));
    return (result.rowCount || 0) > 0;
  }

  async getTransactionsByUserId(userId: number): Promise<Array<Transaction & { asset: Asset }>> {
  ...
  async getPricesByUserId(userId: number): Promise<Array<Price & { asset: Asset }>> {
    const results = await db.select()
      .from(prices)
  ...
  async getLatestPrices(userId: number): Promise<Array<Price & { asset: Asset }>> {
  ...
  async getLatestPriceForAsset(assetId: number, userId: number): Promise<Price | null> {
    const [priceData] = await db.select()
      .from(prices)
      .where(and(
        eq(prices.assetId, assetId),
  ...
  async createPrice(price: InsertPrice & { userId: number }): Promise<Price> {
    const [newPrice] = await db.insert(prices).values(price).returning();
    return newPrice;
  }

  async updatePrice(id: number, userId: number, updates: Partial<InsertPrice>): Promise<Price | undefined> {
  ...
  async deletePricesForAsset(assetId: number, userId: number): Promise<boolean> {
    const result = await db.delete(prices).where(and(
      eq(prices.assetId, assetId), 
      eq(prices.userId, userId)
    ));
    return (result.rowCount || 0) > 0;
  ...
  async getDividendsByUserId(userId: number): Promise<Array<Dividend & { asset: Asset }>> {
    const results = await db.select()
      .from(dividends)
  ...
  async createDividend(dividend: InsertDividend & { userId: number }): Promise<Dividend> {
    const [newDividend] = await db.insert(dividends).values(dividend).returning();
    return newDividend;
  }

  async updateDividend(id: number, userId: number, updates: Partial<InsertDividend>): Promise<Dividend | undefined> {
  ...
  async deleteDividend(id: number, userId: number): Promise<boolean> {
    const result = await db.delete(dividends).where(and(eq(dividends.id, id), eq(dividends.userId, userId)));
    return (result.rowCount || 0) > 0;
  }

  async getCashMovementsByUserId(userId: number): Promise<CashMovement[]> {
    return await db.select().from(cashMovements)
      .where(eq(cashMovements.userId, userId))
      .orderBy(desc(cashMovements.date));
  }

  async createCashMovement(cashMovement: InsertCashMovement & { userId: number }): Promise<CashMovement> {
    const [newCashMovement] = await db.insert(cashMovements).values(cashMovement).returning();
    return newCashMovement;
  }

  async updateCashMovement(id: number, userId: number, updates: Partial<InsertCashMovement>): Promise<CashMovement | undefined> {
  ...
  async deleteCashMovement(id: number, userId: number): Promise<boolean> {
    const result = await db.delete(cashMovements).where(and(eq(cashMovements.id, id), eq(cashMovements.userId, userId)));
    return (result.rowCount || 0) > 0;
  }

  async getPortfolioOverviewForDate(userId: number, snapshotDate: Date): Promise<{
    totalValue: number;
    totalInvested: number;
    totalPL: number;
    totalPLPercent: number;
    ...
    const transactionSummary = await db
      ...
      .where(and(
        ...
        sql`${transactions.date} <= ${snapshotDate.toISOString().split('T')[0]}`
      ))
    ...
    const latestPricesQuery = await db
      ...
      .where(and(
        eq(prices.userId, userId),
        sql`${prices.date} <= ${snapshotDate.toISOString().split('T')[0]}`
      ));
    ...
    const [ytdResult] = await db
      ...
      .where(and(
        ...
        sql`extract(year from ${dividends.paymentDate}) = ${currentYear}`,
        sql`${dividends.paymentDate} <= ${snapshotDate.toISOString().split('T')[0]}`
      ));
  ...
  async getDividendsSummary(userId: number): Promise<{
    ytd: number;
    thisMonth: number;
    avgMonthly: number;
  }> {
    ...
    const [thisMonthResult] = await db
      ...
      .where(and(
        eq(dividends.userId, userId),
        sql`extract(year from ${dividends.paymentDate}) = ${currentYear}`,
        sql`extract(month from ${dividends.paymentDate}) = ${currentMonth}`
      ));
    ...
    const [avgResult] = await db
      ...
      .where(and(
        eq(dividends.userId, userId),
        sql`${dividends.paymentDate} >= current_date - interval '12 months'`
      ));
  ...
  async getClosedPositions(userId: number): Promise<Array<{
    asset: Asset;
    totalBought: number;
    totalSold: number;
    avgBuyPrice: number;
    ...
    firstBuyDate: string;
    lastSellDate: string;
    cycleId: string; // Unique identifier for each buy-sell cycle
  }>> {
      ...
      for (const transaction of sortedTransactions) {
        ...
        if (transaction.type === 'buy') {
          ...
          if (currentPosition > 0 && currentCycleBuys.length > 0) {
            ...
            const totalBoughtInCycle = currentCycleBuys.reduce((sum: number, t: any) => 
              sum + Number(t.quantity), 0);
            
            // Calculate values for this closed portion
            const totalBuyValue = currentCycleBuys.reduce((sum: number, t: any) => 
              sum + (Number(t.quantity) * Number(t.unitPrice)), 0);
            const totalBuyFees = currentCycleBuys.reduce((sum: number, t: any) => 
              sum + Number(t.fees), 0);
            ...
            const holdingPeriod = Math.floor(
              (new Date(transaction.date).getTime() - new Date(cycleStartDate).getTime()) / 
              (1000 * 60 * 60 * 24)
            );
    ...
    return closedPositions.sort((a, b) => 
      new Date(b.lastSellDate).getTime() - new Date(a.lastSellDate).getTime()
    );
  }
  ...
  async createPortfolioSnapshot(snapshot: InsertPortfolioSnapshot & { userId: number }): Promise<PortfolioSnapshot> {
    ...
    for (const holding of overview.holdings) {
      ...
      if (category === 'stock') normalizedCategory = 'stocks';
      else if (category === 'fund') normalizedCategory = 'bonds';
      ...
      if (normalizedCategory in categoryValues) {
        categoryValues[normalizedCategory as keyof typeof categoryValues] += holding.marketValue;
        categoryInvested[normalizedCategory as keyof typeof categoryInvested] += holding.bookValue;
        categoryDetails[normalizedCategory as keyof typeof categoryDetails][holding.asset.name] = holding.marketValue;
      }
    }
  ...
  async updatePortfolioSnapshot(id: number, userId: number, updates: Partial<InsertPortfolioSnapshot>): Promise<PortfolioSnapshot | undefined> {
  ...
  async deletePortfolioSnapshot(id: number, userId: number): Promise<boolean> {
  ...
  async getAssetSnapshots(userId: number, assetId: number): Promise<AssetSnapshot[]> {
  ...
  async createAssetSnapshot(snapshot: InsertAssetSnapshot & { userId: number }): Promise<AssetSnapshot> {
    const [newSnapshot] = await db.insert(assetSnapshots).values(snapshot).returning();
    return newSnapshot;
  }

  async updateAssetSnapshot(id: number, userId: number, updates: Partial<InsertAssetSnapshot>): Promise<AssetSnapshot | undefined> {
  ...
  async deleteAssetSnapshot(id: number, userId: number): Promise<boolean> {
    const result = await db.delete(assetSnapshots)
      .where(and(eq(assetSnapshots.id, id), eq(assetSnapshots.userId, userId)));
    return (result.rowCount || 0) > 0;
  }

  async generateAssetSnapshots(userId: number, assetId: number): Promise<AssetSnapshot[]> {
    ...
    while (currentDate <= endDate) {
      ...
      const snapshot: InsertAssetSnapshot & { userId: number } = {
        userId,
        assetId,
        date: dateStr,
  ...
  async deleteClosedPosition(userId: number, cycleId: string): Promise<boolean> {
